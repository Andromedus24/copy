...
serve(async (req) => {
  ...
  try {
    ...
    // First, analyze the clothing image with OpenRouter Vision API
    console.log('Analyzing clothing image with OpenRouter Vision...');
    ...
    console.log('Generating clothing try-on with OpenRouter...');
    ...
    console.log('Clothing try-on generated successfully');
    ...
    console.log('Clothing try-on saved to wardrobe successfully');
    ...
    console.error('Error in generate-avatar-openrouter function:', error);
    return new Response(JSON.stringify({ 
      error: error.message || 'Failed to generate clothing try-on' 
    }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
